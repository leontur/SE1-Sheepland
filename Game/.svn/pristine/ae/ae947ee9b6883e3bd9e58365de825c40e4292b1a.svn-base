 ...
 * @author Leonardo
 */
public class Wolf extends SelectableObject {
	...
	private ImageIcon imgUp, imgDown, imgLeft, imgRight;
	...
	public Wolf(){		
		try{
			//get files
			imgUp = new ImageIcon(ImageIO.read(new File(dirPath + "wolf.png")));
			imgDown = new ImageIcon(ImageIO.read(new File(dirPath + "wolf.png")));
			imgLeft = new ImageIcon(ImageIO.read(new File(dirPath + "wolf.png")));
			imgRight = new ImageIcon(ImageIO.read(new File(dirPath + "wolf.png")));
			//scaling
			imgUp = new ImageIcon(imgUp.getImage().getScaledInstance(boundX, boundY, Image.SCALE_AREA_AVERAGING));
			imgDown = new ImageIcon(imgDown.getImage().getScaledInstance(boundX, boundY, Image.SCALE_AREA_AVERAGING));
	...
	private void setRandomInitialOrientation(){
		...
		if(casualDirection < 0.25){
			setIcon(imgUp);
		} else if(casualDirection < 0.5) {
			setIcon(imgDown);
		} else if(casualDirection < 0.75) {
			setIcon(imgLeft);
		} else {
			setIcon(imgRight);
		}
